  ...
  return (
    <PortfolioFrame>
      <section className="mx-auto max-w-5xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
        ...
        <h1 className="mt-4 max-w-4xl text-4xl font-bold tracking-tight text-foreground sm:text-6xl">Building useful AI systems with visible boundaries.</h1>
        ...
        <section className="mt-16"><SectionLabel>Working principles</SectionLabel><div className="mt-6 grid gap-4 sm:grid-cols-2">{principles.map((principle, index) => <article key={principle} className="rounded-xl border border-border bg-surface p-5"><span className="font-mono text-xs text-cyan-primary">0{index + 1}</span><p className="mt-4 text-sm leading-7 text-foreground-muted">{principle}</p></article>)}</div></section>
      </section>
    </PortfolioFrame>
  );
}
