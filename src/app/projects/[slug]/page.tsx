...
const evidenceDescriptions: Record<EvidenceLink["kind"], string> = {
  ...
  submission: "A public hackathon submission page for project context.",
};
...
function EvidenceIcon({ kind }: { kind: EvidenceLink["kind"] }) {
  ...
  if (kind === "submission") return <BadgeCheck className="h-4 w-4" aria-hidden="true" />;
  ...
  return (
    <PortfolioFrame>
      <article className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-20 lg:px-8">
        <Link href="/projects" className="inline-flex min-h-11 items-center gap-2 font-mono text-xs text-foreground-muted hover:text-blueprint-primary focus-visible:ring-2 focus-visible:ring-blueprint-primary">
          <ArrowLeft className="h-3.5 w-3.5" aria-hidden="true" /> Back to projects
        </Link>

        <header className="atlas-card atlas-featured-card mt-10 p-6 sm:mt-14 sm:p-10">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            {project.evidence.slice(0, 2).map((item) => (
              <a key={item.href} href={item.href} target="_blank" rel="noreferrer noopener" className="inline-flex min-h-11 items-center gap-2 rounded-md border border-border-strong bg-surface/70 px-4 py-3 font-mono text-xs font-bold text-foreground transition-colors hover:border-blueprint-primary/60 hover:text-blueprint-primary focus-visible:ring-2 focus-visible:ring-blueprint-primary">
        ...
        <div className="grid gap-12 py-12 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-16">
          <div className="space-y-14">
            ...
            <section aria-labelledby="build-heading" className="atlas-index-rail pl-5 sm:pl-7">
              ...
              <dl className="mt-7 grid gap-4 sm:grid-cols-2">
                <div className="atlas-card rounded-lg p-4">
            ...
            <section aria-labelledby="outcome-heading" className="atlas-index-rail pl-5 sm:pl-7">
            ...
            <section aria-labelledby="boundary-heading" className="atlas-card border-thermal-primary/35 bg-thermal-primary/5 p-6">
          ...
          <aside className="atlas-card h-fit space-y-10 p-6 lg:sticky lg:top-28 lg:pt-7">
            <section aria-labelledby="evidence-heading">
              ...
              <div className="mt-4 space-y-3">
                {project.evidence.length > 0 ? project.evidence.map((item) => (
                  <a key={item.href} href={item.href} target="_blank" rel="noreferrer noopener" className="atlas-card atlas-card-hover block rounded-lg p-4 focus-visible:ring-2 focus-visible:ring-blueprint-primary">
                    ...
                    <span className="mt-2 block font-mono text-[10px] uppercase tracking-wider text-blueprint-primary">{evidenceKindLabels[item.kind]}</span>
            ...
            <section aria-labelledby="stack-heading">
              ...
              <ul className="mt-4 flex flex-wrap gap-2">
                {project.stack.map((technology) => <li key={technology} className="rounded border border-border bg-surface/75 px-2.5 py-1.5 font-mono text-[10px] text-foreground-muted">{technology}</li>)}
              </ul>
            </section>
          </aside>
        </div>
        ...
        {relatedProjects.length > 0 ? (
          <section aria-labelledby="related-heading" className="border-t border-border pt-10">
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-2">
              ...
              <Link key={related.slug} href={`/projects/${related.slug}`} className="atlas-card atlas-card-hover group rounded-xl p-5 focus-visible:ring-2 focus-visible:ring-blueprint-primary">
                  ...
                  <h3 className="mt-4 text-lg font-bold text-foreground group-hover:text-blueprint-primary">{related.title}</h3>
